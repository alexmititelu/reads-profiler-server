...
public class BooksController {
    public Integer getID(String genre,String authorName,String title,Integer year,Integer isbn,Integer rating)
    {
        Connection con = Database.getConnection();
        try {
            Statement stmt = con.createStatement();
            String query="select books.ID from books join authors WHERE NAME='"+authorName+"' AND genre='"+genre+"' AND title='"+title+"' AND year="+year.toString()+" and isbn="+isbn.toString()+" and rating="+rating.toString();
    ...
    public void list(PrintWriter out){
            ...
            try {
                Statement stmt = con.createStatement();
                ResultSet rs = stmt.executeQuery("select GENRE,NAME,TITLE,YEAR,ISBN,RATING from books join authors");
                while (rs.next()) {
    ...
    public void addRating(Integer rating,Integer isbn)
    {
        ...
        try {
            PreparedStatement pstmt = con.prepareStatement("update books set rating = (rating * nrrating + ?)/(nrrating + 1), nrrating = nrrating + 1 where isbn =?");
